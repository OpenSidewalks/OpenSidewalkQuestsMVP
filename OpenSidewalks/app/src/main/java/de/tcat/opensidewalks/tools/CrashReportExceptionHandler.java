package de.tcat.opensidewalks.tools;
...
import de.tcat.opensidewalks.util.StreamUtils;
...
public class CrashReportExceptionHandler implements Thread.UncaughtExceptionHandler
{
	...
	private final static String MAILTO = "uwtcat@uw.edu";
	...
	public void askUserToSendCrashReportIfExists(Activity activityCtx)
	{
		if(hasCrashReport())
		{
			...
			deleteCrashReport();
			askUserToSendErrorReport(activityCtx, de.tcat.opensidewalks.R.string.crash_title, reportText);
		}
	}
	...
	private void askUserToSendErrorReport(final Activity activityCtx, final int titleResourceId, String error)
	{
		...
		new Handler(Looper.getMainLooper()).post(new Runnable() { @Override public void run()
		{
			new AlertDialogBuilder(activityCtx)
					...
					.setMessage(de.tcat.opensidewalks.R.string.crash_message)
					.setPositiveButton(de.tcat.opensidewalks.R.string.crash_compose_email, new DialogInterface.OnClickListener()
					{
	...
	private void sendEmail(Activity activityCtx, String text)
	{
		...
		{
			Toast.makeText(activityCtx, de.tcat.opensidewalks.R.string.no_email_client, Toast.LENGTH_SHORT).show();
		}
	}
}
